...
// 💡 [핵심] 10개 사이트 전부 1:1 전용 날짜 해독기 독립 배정 완료!
const extractors = {
  naver: (text: string) => { 
    if (!text) return null;
  ...
  burgerking: (text: string) => { 
    ...
    const endPart = text.includes('~') ? text.split('~')[1] : text;
    const match = endPart.match(/(20\d{2})[.\-/\s]+(\d{1,2})[.\-/\s]+(\d{1,2})/);
  ...
  trip: (text: string) => { 
    // 여행사는 문장 전체에서 날짜 형식을 강제로 뜯어냄
    if (!text || text.includes('소진')) return null;
    let match = text.match(/(20\d{2})[.\-/\s년]+(\d{1,2})[.\-/\s월]+(\d{1,2})/);
    if (match) return `${match[1]}-${match[2].padStart(2, '0')}-${match[3].padStart(2, '0')}`;
    match = text.match(/(\d{1,2})[.\-/\s월]+(\d{1,2})[일]*/);
  ...
  myrealtrip: (text: string) => { 
    if (!text || text.includes('소진')) return null;
    const endPart = text.includes('~') ? text.split('~')[1] : text;
    let match = endPart.match(/(20\d{2})[.\-/\s]+(\d{1,2})[.\-/\s]+(\d{1,2})/);
    if (match) return `${match[1]}-${match[2].padStart(2, '0')}-${match[3].padStart(2, '0')}`;
    match = endPart.match(/(\d{1,2})[.\-/\s월]+(\d{1,2})[일]*/);
    if (match) return `${new Date().getFullYear()}-${match[1].padStart(2, '0')}-${match[2].padStart(2, '0')}`;
    return null;
  },
  cu: (text: string) => { 
    ...
    let match = endPart.match(/(20\d{2})[.\-/\s]+(\d{1,2})[.\-/\s]+(\d{1,2})/);
    if (match) return `${match[1]}-${match[2].padStart(2, '0')}-${match[3].padStart(2, '0')}`;
    match = endPart.match(/(\d{1,2})[.\-/\s]+(\d{1,2})/);
    if (match) return `${new Date().getFullYear()}-${match[1].padStart(2, '0')}-${match[2].padStart(2, '0')}`;
    return null;
  },
...
export async function GET() {
  console.log("🤖 [10개 사이트 전용 해독기 풀가동] 크롤러 시작...");
      ...
      if (title && title.includes('프로모션')) {
        ...
        if (!existingTitles.includes(`[버거킹] ${title}`)) {
          ...
          scrapedDeals.push({
            ...
            category: "음식", sub_category: "버거킹", author: "AutoBot", mall_name: "버거킹", status: "진행중", end_date: extractors.burgerking(rawDate),
          });
        }
      }
    });
      ...
      if (title) {
        ...
        if (!existingTitles.includes(`[T멤버십] ${title}`)) {
          scrapedDeals.push({
            ...
            category: "쇼핑", sub_category: "통신사혜택", author: "AutoBot", mall_name: "SKT", status: "진행중", end_date: extractors.skt(rawDate), 
          });
        }
      }
    });
  ...
  // ====================================================================
  // 4. 여행 3사 (트립, 호텔스, 마이리얼트립 전용 해독기 적용)
  // ====================================================================
  try {
      ...
      if (title && title.length > 5) {
        ...
        if (!existingTitles.includes(`[트립닷컴] ${title}`)) {
          scrapedDeals.push({ 
            ...
            end_date: extractors.trip(rawText)
          });
        }
      }
    });
      ...
      if (title && (title.includes('할인') || title.includes('특가'))) {
        ...
        if (!existingTitles.includes(`[호텔스닷컴] ${title}`)) {
          scrapedDeals.push({ 
            ...
            end_date: extractors.hotels(rawText)
          });
        }
      }
    });
  ...
  try {
    ...
    $('.promotion-item, a[href*="/promotions/"]').each((index, element) => {
      ...
      const rawDate = $(element).find('.date, .period').text().trim() || $(element).text();
      if (title && title.length > 5) {
        totalScrapedCount++;
        if (!existingTitles.includes(`[마이리얼트립] ${title}`)) {
          scrapedDeals.push({ 
            ...
            end_date: extractors.myrealtrip(rawDate) 
          });
        }
      }
    });
    ...
    $('.event_list li, .relm_list li, .info_event li, table tbody tr').each((index, element) => {
      ...
      if (title && title.length > 2) {
        ...
        if (!existingTitles.includes(`[CU] ${title}`)) {
          scrapedDeals.push({
            title: `[CU] ${title}`, content: "CU 편의점 공식 홈페이지 또는 포켓CU 앱에서 혜택을 확인하세요.", url: rawLink ? (rawLink.startsWith('http') ? rawLink : `https://cu.bgfretail.com${rawLink}`) : CU_URL, category: "음식", sub_category: "편의점", author: "AutoBot", mall_name: "CU", status: "진행중", end_date: extractors.cu(rawDate),
          });
        }
      }
    });
    ...
    $('.promList li, .promotion_list li, div.contArea li').each((index, element) => {
      ...
      if (title && title.length > 2) {
        ...
        if (!existingTitles.includes(`[맥도날드] ${title}`)) {
          scrapedDeals.push({
            title: `[맥도날드] ${title}`, content: "맥도날드 공식 홈페이지 또는 맥딜리버리 앱에서 혜택을 확인하세요.", url: rawLink ? `https://www.mcdonalds.co.kr${rawLink}` : MAC_URL, category: "음식", sub_category: "맥도날드", author: "AutoBot", mall_name: "맥도날드", status: "진행중", end_date: extractors.mcdonalds(rawDate),
          });
        }
      }
    });
    ...
    $('.event_list li, .event-item, div.pd_list_wrap li').each((index, element) => {
      ...
      if (title && title.length > 2) {
        ...
        if (!existingTitles.includes(`[써브웨이] ${title}`)) {
          scrapedDeals.push({
            title: `[써브웨이] ${title}`, content: "써브웨이 공식 홈페이지에서 행사 매장 및 상세 조건을 확인하세요.", url: rawLink ? `https://www.subway.co.kr${rawLink}` : SUB_URL, category: "음식", sub_category: "써브웨이", author: "AutoBot", mall_name: "써브웨이", status: "진행중", end_date: extractors.subway(rawDate),
          });
        }
      }
    });
    ...
    $('.event_list_wrap li, .event-list li, article.event-list li').each((index, element) => {
      ...
      if (title && title.length > 2) {
        ...
        if (!existingTitles.includes(`[도미노피자] ${title}`)) {
          scrapedDeals.push({
            title: `[도미노피자] ${title}`, content: "도미노피자 홈페이지 및 앱에서 방문포장/배달 할인 혜택을 확인하세요.", url: rawLink ? `https://web.dominos.co.kr${rawLink}` : DOMINO_URL, category: "음식", sub_category: "도미노피자", author: "AutoBot", mall_name: "도미노피자", status: "진행중", end_date: extractors.dominos(rawDate),
          });
        }
      }
    });
  ...
  console.log(`🎉 [모든 사이트 1:1 해독 완료] 새로운 글 ${newCount}개 추가됨.`);
