...
export async function GET() {
  console.log("🤖 [쇼핑 탭 집결] 4대 혜택 가동 시작...");
    ...
    if (naverData?.elements) {
      naverData.elements.forEach((item: any) => {
        ...
        if (!scrapedDeals.some(deal => deal.title === title)) {
          scrapedDeals.push({
            title: title, content: detailContent, url: item.detailUrl || item.link || "https://pay.naver.com", 
    ...
    $('table.board_list tbody tr').each((index, element) => {
      ...
      if (title) {
        scrapedDeals.push({
          ...
          category: "쇼핑", // 👈 쇼핑 탭으로 이동
          sub_category: "CU", // 👈 말머리 변경
          author: "AutoBot", mall_name: "CU", status: "진행중",
    ...
    $('.event_list li').each((index, element) => {
      ...
      if (title) {
        scrapedDeals.push({
          ...
          category: "쇼핑", // 👈 쇼핑 탭으로 이동
          sub_category: "카드혜택", // 👈 말머리 변경
          author: "AutoBot", mall_name: "카드고릴라", status: "진행중",
          end_date: parseSafeEndDate(rawDate),
        });
      }
    });
    ...
    $('.event_list_wrap ul li').each((index, element) => {
      ...
      if (title) {
        scrapedDeals.push({
          ...
          category: "쇼핑", // 👈 쇼핑 탭으로 이동
          sub_category: "통신사혜택", // 👈 말머리 변경
          author: "AutoBot", mall_name: "SKT", status: "진행중",
          end_date: parseSafeEndDate(rawDate), 
        });
      }
    });
